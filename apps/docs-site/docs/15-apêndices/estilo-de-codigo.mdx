...
## 0) Convenções editoriais

- Preferir **frontmatter completo** em toda página: `id`, `title` e, quando a página participa da ordem da seção, `sidebar_position`.
- Preferir **links relativos** entre documentos (`./` e `../`) em vez de hardcode de rotas `/docs/...`, porque o site roda com `baseUrl: "/devdocs/"`.
- Quando o **nome do arquivo for histórico**, explicitar isso logo no início da página com uma nota curta em bloco de citação.
- Sempre documentar **estado real**, inclusive passivos e desvios conhecidos, em vez de estado desejado.
...
```bash title="Subindo ambiente de desenvolvimento com o helper oficial"
